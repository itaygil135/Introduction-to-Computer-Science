...
class GameRunner:
    ...
    def _game_loop(self):
        ...
        if len(self._astro_list) == 0:
            self.__screen.show_message("end of the game", "you won, no asteroids left")
            self.__screen.end_game()
            sys.exit()


        if self._count_loops > 200:
        ...
        for cur_ast in self._astro_list:
            ...
            if cur_ast.has_intersection(self.my_ship):
                if self._ship_smashed(cur_ast) == "end of life":
                    self.__screen.show_message("end of the game","you lost! you lost all your life")
                    self.__screen.end_game()
                    sys.exit()
                continue
            ...
            for torpedo in self._torpedo_list:
                if cur_ast.has_intersection(torpedo):
                    self._score = self._score + SCORE_DICT[cur_ast.get_ast_size()]
        ...
        if self.__screen.should_end():
            self.__screen.show_message("end of the game", "you chose to quit")
            self.__screen.end_game()
            sys.exit()
